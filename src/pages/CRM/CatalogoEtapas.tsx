...
export default function CatalogoEtapas() {
  ...
  const [form, setForm] = useState<EtapaPayload>({
    nombre: "",
  ...
  const cargar = useCallback(async () => {
    ...
    setError(null);
    try {
      setEtapas(await crmService.getEtapas());
    } catch (err) {
      console.error(err);
      setError(
        err instanceof Error ? err.message : "No se pudieron cargar las etapas.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const guardar = async () => {
    ...
    try {
      ...
      if (editando) {
        await crmService.actualizarEtapa(editando.idEtapa, payload);
      } else {
        await crmService.crearEtapa(payload);
      }
      cerrarModal();
      await cargar();
    } catch (err) {
  ...
  const eliminar = async (row: Etapa) => {
    if (!window.confirm(`¿Eliminar la etapa "${row.nombre}"?`)) return;
    setError(null);
    try {
      await crmService.eliminarEtapa(row.idEtapa);
      await cargar();
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="space-y-6 p-6">
      <PageMeta
        ...
        description="Administre las etapas del funnel de ventas."
      />
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            Configure las etapas del funnel por las que pasan los prospectos.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
        ...
        {loading ? (
          ...
          <p className="px-4 py-12 text-center text-sm text-gray-500 dark:text-gray-400">
            No hay etapas del funnel registradas.
          </p>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {etapas.map((row) => (
                  <tr key={row.idEtapa} className="border-t dark:border-gray-800">
