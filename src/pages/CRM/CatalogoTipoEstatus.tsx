...
export default function CatalogoTipoEstatus() {
  ...
  const [lista, setLista] = useState<TipoEstatusCatalogo[]>([]);
  const [modalAbierto, setModalAbierto] = useState(false);
  const [editando, setEditando] = useState<TipoEstatusCatalogo | null>(null);
  const [form, setForm] = useState<TipoEstatusCatalogoPayload>({
    nombre: "",
  ...
  const cargar = useCallback(async () => {
    ...
    setError(null);
    try {
      setLista(await crmService.getTipoEstatusCatalogo());
    } catch (err) {
      console.error(err);
      setError(
        err instanceof Error
          ? err.message
          : "No se pudieron cargar los tipos de estatus.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const abrirEditar = (row: TipoEstatusCatalogo) => {
  ...
  const guardar = async () => {
    ...
    try {
      ...
      if (editando) {
        await crmService.actualizarTipoEstatusCatalogo(
          editando.idTipoEstatus,
          payload,
        );
      } else {
        await crmService.crearTipoEstatusCatalogo(payload);
      }
      cerrarModal();
      await cargar();
    } catch (err) {
  ...
  const eliminar = async (row: TipoEstatusCatalogo) => {
    if (!window.confirm(`¿Eliminar el tipo de estatus "${row.nombre}"?`)) return;
    setError(null);
    try {
      await crmService.eliminarTipoEstatusCatalogo(row.idTipoEstatus);
      await cargar();
    } catch (err) {
      console.error(err);
      ...
      <div className="rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
        ...
        {loading ? (
          ...
          <p className="px-4 py-12 text-center text-sm text-gray-500 dark:text-gray-400">
            No hay tipos de estatus registrados.
          </p>
        ) : (
        ...
        >
          <div className="w-full max-w-md rounded-lg border border-gray-200 bg-white p-5 shadow-xl dark:border-gray-600 dark:bg-gray-800">
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
              {editando ? "Editar tipo de estatus" : "Nuevo tipo de estatus"}
            </h3>
